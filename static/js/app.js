var map, featureList, boroughSearch = [], theaterSearch = [], museumSearch = [], sitiSearch = [];
...
function sidebarClick(id) {
  ...
  // se il livello di zoom rimane basso (fintanto ho il cluster) => non si apre il popup
...
function syncSidebar() {
  ...
  museums.eachLayer(function (layer) {
   ...
   /* Loop through museums layer and add only features which are in the map bounds */
  siti.eachLayer(function (layer) {
    if (map.hasLayer(sitiLayer)) {
      if (map.getBounds().contains(layer.getLatLng())) {
        $("#feature-list tbody").append(
          '<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '">'+
          '<td style="vertical-align: middle;"><i class="fa fa-pagelines" aria-hidden="true"></td>'+
...
$.getJSON("static/data/teatri.geojson", function (data) {
  theaters.addData(data);
  // determina se questo è il layer di default
...
$.getJSON("static/data/musei.geojson", function (data) {
...
var sitiLayer = L.geoJson(null);
var siti = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      /////////////////////////// questo gestisce il MODALE  ///////////////////////////
      // var content = "<table class='table table-striped table-bordered table-condensed'>" +
      //               "<tr><th>Name</th><td>" + feature.properties.name + "</td></tr>" +
      //               "<tr><th>ID</th><td>" + feature.properties.id + "</td></tr>" +
      //               "<table>";
      // var contentFooter = '<button onClick="getDetails(\'' +feature.properties.id+ '\')" class="btn btn-default pull-left"><i class="fa fa-info"></i>&nbsp;Dettagli</button>'
      // layer.on({
      //   click: function (e) {
      //     $("#feature-title").html(feature.properties.name);
      //     $("#feature-info").html(content);
      //     $("#feature-details").html(contentFooter);
      //     $("#featureModal").modal("show");
      //     highlight.clearLayers().addLayer(L.circleMarker([feature.geometry.coordinates[1], feature.geometry.coordinates[0]], highlightStyle));
      //   }
      // });

      /////////////////////////// questo gestisce il POPUP  ///////////////////////////
        layer.bindPopup(
              '<h5><strong>'+feature.properties.name+'</strong></h5>'+
              '<table class="table table-striped table-condensed">'+
              '<tr><td>Specie</td><td>'+feature.properties.id+'</td></tr>'+
              '<tr><td>Data rilievo</td><td>'+feature.properties.data_rilievo+'</td></tr>'+
              '<tr><td>Rilevatore</td><td>'+feature.properties.rilevatore+'</td></tr>'+
              '</table>'+
              '<button onClick="getDetails(\'' +feature.properties.id+ '\')" class="btn btn-xs btn-default"><i class="fa fa-info"></i>&nbsp;Dettagli</button>'
         );
        layer.on({
          click: function (e) {
            highlight.clearLayers().addLayer(L.circleMarker([feature.geometry.coordinates[1], feature.geometry.coordinates[0]], highlightStyle));
      ...
      // // LISTA delle FEATURES a SX
      // APPARENTEMENTE NON SERVE QUI !!!!
      // $("#feature-list tbody").append(
      //     '<tr class="feature-row" id="' + L.stamp(layer) +
      //     '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '">' +
      //       '<td style="vertical-align: middle;"><i class="fa fa-pagelines" aria-hidden="true"></i></td>' +
      //       '<td class="feature-name">' + layer.feature.properties.name + '</td>' +
      //       '<td style="vertical-align: middle;"><i class="fa fa-pagelines pull-right"></i></td>' +
      //     '</tr>'
      //   );
...
$.getJSON("/engine/getpoints", function (data) {
  siti.addData(data);
  map.addLayer(sitiLayer);
});
// $.ajax({
//     url : '/engine/getpoints',
//     dataType : 'json',
...
// Crea Cluster
// var clusterSiti = new L.MarkerClusterGroup({
//   maxClusterRadius: 50,
//   iconCreateFunction: function(cluster) {
...
map = L.map("map", {
  ...
  layers: [CartoDB_Positron, markerClusters, highlight],
  zoomControl: false,
...
map.on("overlayadd", function(e) {
  if (e.layer === sitiLayer) {
    markerClusters.addLayer(siti);
    syncSidebar();
  }
...
map.on("overlayremove", function(e) {
  if (e.layer === sitiLayer) {
    markerClusters.removeLayer(siti);
    syncSidebar();
  }
  if (e.layer === theaterLayer) {
...
var groupedOverlays = {
  "Points of Interest": {
    "<i class='fa fa-pagelines' aria-hidden='true'></i>&nbsp;&nbsp;&nbsp;Siti": sitiLayer,
...
$(document).one("ajaxStop", function () {
  ...
  // },
  {
    ...
    name: "Siti",
    displayKey: "name",
    source: sitiBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='assets/img/museum.png' width='24' height='28'>&nbsp;Siti</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  }
  // , {
